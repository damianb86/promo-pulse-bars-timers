...
import { canUseFeature } from "../services/planLimits.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (!shop) {
      return jsonResponse({ error: "Shop was not found." }, { status: 404 });
    }
    ...
    if (!gate.allowed) {
      return jsonResponse({ error: gate.reason }, { status: 403 });
    }
    ...
    if (input.action === "expire") {
      ...
      return jsonResponse({ ok: true, expired: true }, { status: 200 });
    }
